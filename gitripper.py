...
def download_zip(owner: str, repo: str, ref: str, token: Optional[str], dest_path: Path) -> Path:
    """
    ...
    with requests.get(url, headers=headers, stream=True, timeout=60) as r:
        if r.status_code == 200:
            ...
            raise RuntimeError(f"Unexpected redirect from GitHub archive endpoint: {r.status_code}")
        elif r.status_code == 404:
            raise FileNotFoundError(f"Archive for {owner}/{repo}@{ref} not found (404).")
        else:
            raise RuntimeError(f"Failed to download archive: {r.status_code} {r.text}")
    ...
    for root, dirs, files in os.walk(dirpath):
        if '.git' in dirs:
            ...
            try:
                ...
                print(f"Warning: failed to remove embedded .git at {git_dir}: {e}", file=sys.stderr)
...
def check_git_installed():
    ...
    try:
        ...
        raise EnvironmentError("git is not installed or not available in PATH.") from e


def initialize_repo(dest: Path, author_name: Optional[str], author_email: Optional[str], remote: Optional[str]):
    ...
    if author_name:
        subprocess.run(["git", "config", "user.name", author_name], cwd=str(dest), check=True)
    if author_email:
        subprocess.run(["git", "config", "user.email", author_email], cwd=str(dest), check=True)
    ...
    # commit
    subprocess.run(["git", "commit", "-m", "Initial commit"], cwd=str(dest), check=True)

    # optionally set remote
    if remote:
        subprocess.run(["git", "remote", "add", "origin", remote], cwd=str(dest), check=True)
        print(f"Set remote origin to {remote}")


def main():
    p = argparse.ArgumentParser(description="Download a GitHub repository's contents (no history) and create a local git repo with a single Initial commit.")
    p.add_argument("url", help="URL of the GitHub repository (https://github.com/owner/repo, git@..., etc.)")
    p.add_argument("--branch", help="Branch/ref to fetch (default: repo default branch)", default=None)
    p.add_argument("--token", help="GitHub personal access token (also read from GITHUB_TOKEN env var)", default=None)
    p.add_argument("--dest", help="Destination directory (default: ./<repo>-copy)", default=None)
    p.add_argument("--author-name", help="Set git user.name for the initial commit", default=None)
    p.add_argument("--author-email", help="Set git user.email for the initial commit", default=None)
    p.add_argument("--remote", help="Set git remote origin (e.g. a new repo URL) after initial commit", default=None)
    p.add_argument("--force", help="Overwrite destination if it exists", action="store_true")
    args = p.parse_args()
    ...
    if dest.exists():
        if any(dest.iterdir()) and not args.force:
            print(f"Destination '{dest}' exists and is not empty. Use --force to overwrite.", file=sys.stderr)
            sys.exit(3)
        elif args.force:
            # remove and recreate
            try:
                ...
                print(f"Failed to remove existing destination: {e}", file=sys.stderr)
                sys.exit(4)
    ...
    if ref is None:
        try:
            ...
            print(f"Warning: could not determine default branch via API: {e}. Falling back to 'main'.")
            ref = "main"
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            print(f"Failed to download repository archive: {e}", file=sys.stderr)
            sys.exit(6)
    ...
    print("Note: this repository has no history from the original repo. Check license of original before redistribution.")
